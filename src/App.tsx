...
import { ArrowDownToLine, ArrowRight, Check, CheckCheck, Monitor, Moon, Server, Settings, Sun, X } from 'lucide-react';
import Artwork, { Cube, FabricIcon, GrassBlock } from './Artwork';
import { mods } from './data';
type Tab = 'mods' | 'packs' | 'servers';
...
function Header({
  ...
  return <header className="header"><div className="header-inner"><a className="brand" href="#" onClick={e => {
        e.preventDefault();
        setTab('mods');
      }} aria-label="Server Mods 首页"><Cube /><span>Server Mods</span></a><nav aria-label="主导航">{([['mods', '模组'], ['packs', '整合包'], ['servers', '服务器']] as const).map(([key, label]) => <button key={key} className={tab === key ? 'nav-link active' : 'nav-link'} aria-current={tab === key ? 'page' : undefined} onClick={() => setTab(key)}>{label}</button>)}</nav><div className="header-actions"><button className="manage-button" onClick={manage}><Settings size={18} /><span>管理</span></button><div className="theme-control"><Sun size={19} /><button className="theme-switch" role="switch" aria-checked={theme === 'dark'} aria-label="深色主题" onClick={toggleTheme}><span /></button><Moon size={18} /></div></div></div></header>;
}
function Hero() {
...
function DemoPanel({
  ...
  tab: Exclude<Tab, 'mods'>;
  download: (name: string) => void;
  manage: () => void;
}) {
  return <section className="demo-panel"><div className="demo-heading"><span className="section-kicker">{tab === 'packs' ? 'CURATED FOR YOUR WORLD' : 'BETTER TOGETHER'}</span><h2>{tab === 'packs' ? '为冒险，准备就绪。' : '让下一个世界，在这里开始。'}</h2><p>{tab === 'packs' ? '精选轻量模组，为原版体验带来恰到好处的提升。' : '和朋友一起，探索更多可能。'}</p></div><div className="demo-card"><div className="demo-symbol">{tab === 'packs' ? <Cube /> : <Server size={34} />}</div><div><h3>{tab === 'packs' ? '原版增强整合包' : '我的 Minecraft 服务器'}</h3><p>Minecraft 1.20.1 · Fabric 0.15.11</p><div className="demo-tags"><span><Check size={13} />{tab === 'packs' ? '6 个精选模组' : '本地演示'}</span><span>{tab === 'packs' ? '轻量 · 流畅 · 兼容' : '尚未连接服务器'}</span></div></div><button className="icon-button" aria-label={tab === 'packs' ? '下载原版增强整合包' : '打开服务器设置'} onClick={() => tab === 'packs' ? download('原版增强整合包') : manage()}>{tab === 'packs' ? <ArrowDownToLine /> : <ArrowRight />}</button></div><p className="demo-note">此页面为界面演示，{tab === 'packs' ? '暂未提供真实下载资源。' : '暂未接入服务器管理服务。'}</p></section>;
}
...
export default function App() {
  ...
  return <><Header tab={tab} setTab={setTab} theme={theme} toggleTheme={toggleTheme} manage={() => setSettings(true)} /><main><Artwork /><div className="main-content"><Hero />{tab === 'mods' ? <><DownloadCard download={download} /><ModList download={download} /></> : <DemoPanel tab={tab} download={download} manage={() => setSettings(true)} />}<footer><span className="status-dot" />为更好的游戏体验而构建<span className="footer-separator">/</span><span>保持原版，探索更多。</span></footer></div></main><ManageDialog open={settings} close={() => setSettings(false)} theme={theme} toggleTheme={toggleTheme} /><div className="toast-region" role="status" aria-live="polite">{toast && <div className="toast" key={toast.id}><CheckCheck size={22} /><div><strong>{toast.name}</strong><p>演示模式：暂未接入真实下载资源。</p></div><button aria-label="关闭提示" onClick={() => setToast(null)}><X size={18} /></button></div>}</div></>;
}
